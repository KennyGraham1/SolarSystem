...
import { TOUR_IDS, bodyById, bodyHref } from "@/lib/planets";

const TOUR = TOUR_IDS.map(bodyById);
...
export function Tour() {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowRight" && step < TOUR.length - 1) setTourStep(step + 1);
      ...
      <div key={step} className="glass-strong animate-drift-down pointer-events-auto w-full max-w-xl rounded-2xl border-amber-300/25 p-4 sm:p-5">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <Link href={bodyHref(body)} className="text-xs text-white/60 underline-offset-4 transition hover:text-amber-200 hover:underline">
              Read more ↗
            </Link>
