...
import { BODIES, bodyById, type BodyId } from "@/lib/planets";
...
const isBodyId = (id: string): id is BodyId => BODIES.some((b) => b.id === id);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!isBodyId(id)) return { title: "Not found" };
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0">
        <span className="eyebrow block">{dir === "prev" ? "← Sunward" : "Outward →"}</span>
...
export default async function PlanetPage({ params }: Props) {
  const { id } = await params;
  if (!isBodyId(id)) notFound();

  const body = bodyById(id);
  const content = CONTENT[id];
  const index = BODIES.findIndex((b) => b.id === id);
  const prev = index > 0 ? BODIES[index - 1].id : null;
  const next = index < BODIES.length - 1 ? BODIES[index + 1].id : null;
        ...
        <section className="grid items-center gap-8 py-6 lg:grid-cols-[1.05fr_1fr] lg:gap-12 lg:py-10">
          <div className="animate-slide-up">
            <div className="eyebrow mb-3 flex items-center gap-2">
              ...
              {body.type} · {index === 0 ? "the centre" : `${index}${index === 1 ? "st" : index === 2 ? "nd" : index === 3 ? "rd" : "th"} from the Sun`}
            </div>
